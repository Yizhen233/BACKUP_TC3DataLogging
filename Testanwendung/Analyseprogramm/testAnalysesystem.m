...
test=readtable('testTable.csv');                  %csv Datei zu Tabelle
...
tr=timerange(datetime('01/03/2020 12:00','format','dd/MM/yyyy HH:mm'),datetime('30/03/2020 12:00','format','dd/MM/yyyy HH:mm'));
...
%disp(room1Data);
room2Data=timelimitedData(timelimitedData.room == '2',:); 
...
plot(weeklyroom1Data.time,weeklyroom1Data.indoor_T);         %Linie zeigen
ylim([20 30]);                                               %Bereich y-Achse
ylabel('indoor temperature (\circ C)');
yyaxis right;
plot(weeklyroom1Data.time,weeklyroom1Data.tank_T);
ylim([45 55]);
ylabel('watertank temperature (\circ C)');
title('weekly value after controlling of room1 in 03.2020-04.2020');
xlim([min(weeklyroom1Data.time) max(weeklyroom1Data.time)]);
xtickformat('dd-MM');                                              %Form der x-koordinate dd-MM-yyyy HH:mm:ss
xticks(min(weeklyroom1Data.time):7:max(weeklyroom1Data.time));     %x-Achse Wertintervall
...
test2=readtable('testTable2.csv');                  %csv Datei zu Tabelle
...
figure;
yyaxis left;
plot(synroom1Data.time,synroom1Data.indoor_T);
ylim([20 30]);
ylabel('indoor temperature (\circ C)');
yyaxis right;
Proom1outdoor=plot(synroom1Data.time,synroom1Data.outdoor_T,'-o'); %Marker als o, index zu linie geben          
ylim([20 30]);
ylabel('outdoor temperature (\circ C)');
title('indoor and outdoor temperature of room1 in 03.2020-04.2020');
...
for m=1:1:height(synroom1Data)
    if any(synroom1Data.outdoor_T(m,1)>25)                        %mit der Grenzwerte vergleichen
